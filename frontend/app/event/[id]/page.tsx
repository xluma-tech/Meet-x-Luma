...
export default function EventPage() {
  ...
  useEffect(() => {
    // Don't fetch if eventId is not available yet
    if (!eventId || typeof eventId !== 'string' || eventId === 'undefined') {
    ...
    fetch(`${backendUrl}/api/events/${eventId}`)
      .then((res) => {
        if (!res.ok) {
          console.error('[Fetch Event] Backend returned:', res.status);
          throw new Error('Event not found');
        }
        return res.json();
      })
      .then((data) => {
        console.log('[Fetch Event] Success:', data);
        setEvent(data);
        setLoading(false);
      })
      .catch((err) => {
  ...
  const handleJoin = () => {
    if (userName.trim() && eventId) {
      window.location.href = `/room/${eventId}?name=${encodeURIComponent(userName.trim())}`;
    }
  };
